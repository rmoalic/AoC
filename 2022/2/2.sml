val inputFile = "input.txt";

fun stripLast str = String.substring(str, 0, ((String.size str) - 1));

datatype hand = Rock | Paper | Sizor;
type game = hand * hand;
datatype outcome = Win | Lose | Tie;
exception ParseError;

fun charToHand #"A" = Rock
  | charToHand #"B" = Paper
  | charToHand #"C" = Sizor
  | charToHand c = raise ParseError

fun charToOutcome #"X" = Lose
  | charToOutcome #"Y" = Tie
  | charToOutcome #"Z" = Win
  | charToOutcome c = raise ParseError

fun handToScore Rock = 1
  | handToScore Paper = 2
  | handToScore Sizor = 3

fun roundSelectMove (Rock,  Win ) = Paper
  | roundSelectMove (Rock,  Lose) = Sizor
  | roundSelectMove (Sizor, Lose) = Paper
  | roundSelectMove (Sizor, Win )  = Rock
  | roundSelectMove (Paper, Win )  = Sizor
  | roundSelectMove (Paper, Lose) = Rock
  | roundSelectMove (a, Tie) = a

fun outcomeToScore Win = 6
  | outcomeToScore Lose = 0
  | outcomeToScore Tie = 3

fun gameToScore game = outcomeToScore (#2 game) + handToScore (roundSelectMove game)

fun parseInputLine line = let
    val cars = explode line
    val opponent = charToHand (List.hd cars)
    val player = charToOutcome (List.last cars)
in
    (opponent, player)
end

fun parseInputFile file = let
    val inStream = TextIO.openIn file
    fun readLines stream =
        case TextIO.inputLine stream of
            NONE => []
          | SOME line => parseInputLine (stripLast line) :: readLines stream
in
    readLines inStream before TextIO.closeIn inStream
end

val data = parseInputFile inputFile;
val part2 = List.foldl Int.+ 0 (map gameToScore data);
val _ = print ("solution part 2: " ^ (Int.toString part2) ^ "\n");
